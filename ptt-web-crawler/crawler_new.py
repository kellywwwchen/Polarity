...
class PttWebCrawler(object):
    ...
    def parse_articles(self, start, end, board, path='.', timeout=3):
            ...
            # === Mine ===
            filename = 'data/'+ board + '-' + str(start) + '-' + str(end) + '.json'
            # === end ===
    ...
    def parse(link, article_id, board, timeout=3):
        ...
        # === Mine 處理紅底的帳號 ===
        content2 = ' '.join(filtered)
        content2 = re.sub(r'(\s)+', ' ', content2) 
        content = re.sub(r'(\s)+', ' ', filtered[0])   # 真正的content 
        # === end ===
        ...
        # === Mine ===
        if len(main_content.find_all('span', class_='f3 b1 hl')) != 0:
            red_id = [x.text for x in main_content.find_all('span', class_='f3 b1 hl')]
            q = []
            c = content2.split(' ')
            for i in range(len(c)):
                d = {}
                if c[i] in red_id:
                    d['push_content'] = c[i+2]
                    d['push_ipdatetime'] = c[i+3]+' '+c[i+4]+' '+c[i+5]
                    if c[i-1] not in ['推', '噓']:
                        d['push_tag'] = '→' 
                    else:
                        d['push_tag'] = c[i-1]
            ...
            messages.extend(q)
            message_count['all'] = message_count['all']+len(q)
            message_count['push'] = message_count['push'] + q.count('推')
            message_count['boo'] = message_count['push'] + q.count('噓')
            message_count['neutral'] = message_count['push'] + q.count('→')
            message_count['count'] = message_count['push'] - message_count['boo']
